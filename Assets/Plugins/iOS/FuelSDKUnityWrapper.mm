...
#define NORMALIZED_JSON_DATATYPE_STRING	5
    ...
    BOOL iOSIgniteGetEvent(const char* tags)
    {
        ...
        if( tags != nil ) {
            ...
            tagsArray= [NSJSONSerialization JSONObjectWithData:tagsData options:NSJSONReadingMutableContainers error:&error];
    ...
    BOOL iOSIgniteJoinEvent(const char* eventID)
    {
        NSString* eventIDString = [NSString stringWithFormat:@"%s" , eventID];
        
        fuelignite *gSDK = [fuelignite instance];
        return   [gSDK joinEvent:eventIDString];
    }
