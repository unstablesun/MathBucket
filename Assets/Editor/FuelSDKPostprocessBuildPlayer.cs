...
public class FuelSDKPostprocessBuildPlayer: MonoBehaviour
{
	...
	{
		if (File.Exists (path + "/Plugins/FuelSDK.cs"))
		{
			return path;
		}
	...
	public static void OnPostprocessBuild (BuildTarget target, string pathToBuiltProject)
	{
		...
		if (pluginRootPath == null)
		{
			UnityEngine.Debug.Log ("[FuelSDK] Unable to find the plugin root path");
			return;
		}
		
		if (File.Exists (pathToBuiltProject + "/Classes/FuelSDK.h"))
		{
			UnityEngine.Debug.Log ("[FuelSDK] iOS project build already exists, skipping automatic SDK injection");
			return;
		}
		
		UnityEngine.Debug.Log ("[FuelSDK] Setting the post build script permissions...");
		
		ExecutionResult result = Execute (
			"chmod",
			"755",
			"\"" + pluginRootPath + "/Fuel/PostbuildScripts/PostbuildFuelScript\"");
		
		if (result.success)
		{
			UnityEngine.Debug.Log ("[FuelSDK] Setting the post build script permissions succeeded!" + GetExecutionResultOutput (result));
		}
		else
		{
			UnityEngine.Debug.LogError ("[FuelSDK] Setting the post build script permissions failed!" + GetExecutionResultOutput (result));
			return;
		}
		...
		UnityEngine.Debug.Log ("[FuelSDK] Injecting Fuel SDK into the Xcode project...");
		...
		if (result.success)
		{
			UnityEngine.Debug.Log ("[FuelSDK] Injection of the Fuel SDK into the Xcode project succeeded!" + GetExecutionResultOutput (result));
		}
		else
		{
			UnityEngine.Debug.LogError ("[FuelSDK] Injection of the Fuel SDK into the Xcode project failed!" + GetExecutionResultOutput (result));
			return;
		}
		#endif
